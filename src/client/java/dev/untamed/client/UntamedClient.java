...
import dev.untamed.network.TorchRulesPayload;
...
public class UntamedClient implements ClientModInitializer {
     ...
     *
     * <p>A untamed that silently changes a familiar item is just
    ...
    private static String tooltipKeyFor(Item item) {
        if (item == ModBlocks.UNLIT_TORCH_ITEM) {
            ...
            return "tooltip.untamed.torch.unlit";
        }
        if (item == Items.TORCH || item == ModBlocks.TORCH_ITEM) {
            return ServerTorchRules.relightWhenDry()
                    ? "tooltip.untamed.torch.relights"
                    : "tooltip.untamed.torch.manual_relight";
        }
        return null;
    }
}
