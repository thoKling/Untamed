...
import dev.untamed.launcher.infrastructure.manifest.ManifestDocuments
...
class ManifestDocumentsTest {
    ...
    fun `rejects an artifact that is not served over https`() {
        val result = ManifestDocuments.read(
            manifestJson(modUrl = "http://downloads.example.test/untamed-0.1.0.jar"),
            ORIGIN,
            "0.1.0",
        )
    ...
    private fun manifestJson(
        ...
        modUrl: String = "https://downloads.example.test/untamed-0.1.0.jar",
        modFileName: String = "untamed-0.1.0.jar",
    ): String = """
        {
