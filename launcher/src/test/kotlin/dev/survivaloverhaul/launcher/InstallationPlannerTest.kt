...
import dev.survivaloverhaul.launcher.domain.installation.StepAction
...
class InstallationPlannerTest {
    ...
    fun `only the component that moved is updated`() {
        val snapshot = InstallationPlanner.inspect(
            manifest,
            installed.copy(survivalOverhaulVersion = "0.0.9"),
        )

        val plan = InstallationPlanner.plan(snapshot)
        assertEquals(1, plan.steps.size)
        assertEquals(Component.SURVIVAL_OVERHAUL, plan.steps.single().component)
    ...
    fun `a version newer than the manifest is still a change to make`() {
        val snapshot = InstallationPlanner.inspect(
            manifest,
            installed.copy(survivalOverhaulVersion = "0.9.9"),
        )

        assertEquals(
            ComponentState.OUTDATED,
            snapshot.statusOf(Component.SURVIVAL_OVERHAUL)?.state,
        )
    }
    ...
    fun `an uninspected snapshot plans nothing rather than a full reinstall`() {
        val plan = InstallationPlanner.plan(
            dev.survivaloverhaul.launcher.domain.installation.InstallationSnapshot.NOT_INSPECTED,
        )
