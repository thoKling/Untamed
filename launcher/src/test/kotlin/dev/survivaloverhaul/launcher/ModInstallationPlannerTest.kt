...
import dev.survivaloverhaul.launcher.application.installation.ModInstallationPlanner
import dev.survivaloverhaul.launcher.domain.manifest.ResourceArtifact
import dev.survivaloverhaul.launcher.domain.manifest.ResourceKind
...
class ModInstallationPlannerTest {
    ...
    fun `puts the mod and its dependencies in the mods directory`() {
        ...
        assertEquals(
            listOf("mods/fabric-api-0.160.0+26.2.jar", "mods/survivaloverhaul-0.1.0.jar"),
    ...
    fun `installs dependencies before the mod that needs them`() {
        ...
        val mod = plan.mods.requests
            .indexOfFirst { it.destination.endsWith("survivaloverhaul-0.1.0.jar") }
    ...
    fun `marks the previous version's jar for removal`() {
        val plan = ModInstallationPlanner.plan(
            ...
            previouslyInstalled = setOf(
                "mods/survivaloverhaul-0.0.9.jar",
                "mods/fabric-api-0.160.0+26.2.jar",
            ),
        )

        assertEquals(listOf("mods/survivaloverhaul-0.0.9.jar"), plan.obsolete)
    }
    ...
    fun `keeps a file the manifest still asks for`() {
        val plan = ModInstallationPlanner.plan(
            Fixtures.manifest(),
            previouslyInstalled = setOf("mods/survivaloverhaul-0.1.0.jar"),
        )
    ...
    fun `reports an artifact it cannot install instead of installing it`() {
        ...
        assertTrue(
            plan.mods.requests.none { it.destination.endsWith("survivaloverhaul-0.1.0.jar") },
