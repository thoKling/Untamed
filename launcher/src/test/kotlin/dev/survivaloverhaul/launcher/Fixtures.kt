...
import dev.survivaloverhaul.launcher.domain.manifest.Artifact
import dev.survivaloverhaul.launcher.domain.manifest.ReleaseChannel
...
object Fixtures {
    ...
    fun game(): GameConfiguration = GameConfiguration(
        ...
        survivalOverhaulVersion = "0.1.0",
    )

    fun artifact(
        id: String = "survival-overhaul",
        version: String = "0.1.0",
        url: String = "https://downloads.example.test/mod/survivaloverhaul-0.1.0.jar",
        sha256: String = SHA,
        fileName: String = "survivaloverhaul-0.1.0.jar",
    ): Artifact = Artifact(
        id = id,
    ...
    fun manifestWithResourcePack(): SurvivalManifest = manifest(
        resources = listOf(
            ResourceArtifact(
                ...
                artifact = artifact(
                    id = "survival-overhaul-textures",
                    version = "1.0.0",
                    url = "https://downloads.example.test/packs/textures-1.0.0.zip",
                    fileName = "survival-overhaul-textures-1.0.0.zip",
                ),
            ),
        ),
    )
